...
class FretBoardGtrTest(unittest.TestCase):
    ...
    def test_setcolor(self):
        ...
        F = FretBoardGtr()
        F.set_color(perfectfourth='rgb(0, 0, 0)',
                    root='rgb(0, 0, 0)', default_theme=True)
        self.assertEqual(F.dic_color, {
            "1": 'rgb(231, 0, 0)',
            "b2": 'rgb(249, 229, 0)',
            "2": 'rgb(249, 165, 0)',
        ...
        # Call unchanged function.
        F.set_color(perfectourth='rgb(0, 0, 0)', root='rgb(0, 0, 0)')
        sys.stdout = sys.__stdout__   # Reset redirect.
        self.assertEqual('perfectourth is not a valid attribute. It is not taken into accounts.', str(
        ...
        sys.stdout = capturedOutput  # and redirect stdout.
        # Call unchanged function.
        F.set_color(perfectfourth=2, root='rgb(0, 0, 0)')
        sys.stdout = sys.__stdout__   # Reset redirect.
        self.assertEqual('2 (int) is not a good format for perfectfourth attribute, use str instead. Modifications are not taken into accounts.', str(
            capturedOutput.getvalue()).split('\n')[0])
    ...
    def test_createobjectattributs(self):
        ...
        sys.stdout = sys.__stdout__   # Reset redirect.
        self.assertEqual('open_coolor_scale is not a valid attribute. It is not taken into accounts.', str(
            capturedOutput.getvalue()).split('\n')[0])
        ...
        sys.stdout = sys.__stdout__   # Reset redirect.
        self.assertEqual('2 (int) is not a good format for open_color_scale attribute, use bool instead.  Modifications are not taken into accounts.', str(
            capturedOutput.getvalue()).split('\n')[0])
    ...
    def test_notes_names_from_fingering(self):
        ...
        self.skipTest("Tests might not be valid anymore.")
    ...
    def test_setenharmonic(self):
        ...
        self.assertEqual(FretBoardGtr.setenharmonic(
            ['A#', 'C', 'D', 'D#', 'F', 'G', 'A']), ['Bb', 'C', 'D', 'Eb', 'F', 'G', 'A'])
        self.assertEqual(FretBoardGtr.setenharmonic(["A", "A#", "B", "C", "C#", "D", "D#", "E", "F", "F#", "G", "G#"]), [
                         'A', 'A#', 'B', 'C', 'C#', 'D', 'D#', 'E', 'F', 'F#', 'G', 'G#'])
        self.assertEqual(FretBoardGtr.setenharmonic(['F#', 'G#', 'A#', 'B', 'C#', 'D#', 'F']), [
                         'Gb', 'Ab', 'Bb', 'Cb', 'Db', 'Eb', 'F'])
    ...
    def test_where_dot(self):
        ...
        tupledot = F.wheredot()
        self.assertEqual(
            tupledot, ([2, 4, 6, 8, 11, 14, 16], [1, 1, 1, 1, 2, 1, 1]))
