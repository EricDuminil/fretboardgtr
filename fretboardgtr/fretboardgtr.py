...
from fretboardgtr.fingering import Fingering
...
class FretBoardGtr():
    ...
    def fingering(self, value):
        self._fingering = Fingering(value)
            ...
            for keys, value in kwargs.items():
                if keys in dic_name.keys():
                    if type(value) == type(self.dic_color[dic_name[keys]]):
                        ...
                        try:
                            raise ValueError('{} ({}) is not a good format for {} attribute, use {} instead.'.format(
                                value, str(type(value).__name__), keys, str(type(self.dic_color[dic_name[keys]]).__name__)))
                        except Exception as e:
                            print(e, 'Modifications are not taken into accounts.')
                else:
                    try:
                        raise ValueError(
                            '{} is not a valid attribute.'.format(keys))
                    except Exception as e:
            ...
            for keys, value in kwargs.items():
                if keys in self.attribute_dic.keys():
                    if type(value) == type(self.attribute_dic[keys]):
                        ...
                        try:
                            raise ValueError('{} ({}) is not a good format for {} attribute, use {} instead.'.format(
                                value, str(type(value).__name__), keys, str(type(self.attribute_dic[keys]).__name__)))
                        except Exception as e:
                            print(e, ' Modifications are not taken into accounts.')
                else:
                    try:
    ...
    def notesname(self):
        ...
        for string, fret in self.fingering:
            if fret is None:
                ...
                ind = chroma.index(self.tuning[string])
                notes.append(chroma[(ind+fret) % 12])
    ...
    def minmax(self):
        ...
        (2,3)
        '''
        return self.fingering.min, self.fingering.max
    ...
    def save(self, extension=None):
        ...
        * ppm
        ...
        if not extension.strip():
            raise ValueError(
                "Please specify an extension, either in self.path or as argument for save.")
